    ...
    public String getHint(String secret, String guess) {
        ...
        for (int i = 0; i < secret.length(); i++) {
            if (secret.charAt(i) == guess.charAt(i)) bullCount++;
                ...
                if (!s.containsKey(secret.charAt(i))) s.put(secret.charAt(i), 1);
                else s.put(secret.charAt(i), s.get(secret.charAt(i)) + 1);
                if (!g.containsKey(guess.charAt(i))) g.put(guess.charAt(i), 1);
                else g.put(guess.charAt(i), g.get(guess.charAt(i)) + 1);
            }
        }

        for (Character ch : s.keySet()) {
            if (g.containsKey(ch)) {
                cowCount += Math.min(s.get(ch), g.get(ch));
            }
        }
